...
async getProductFromWishlist(id){
  const [rows] = await this.database.query(`SELECT p.*, cat.name AS categoryName FROM product AS p JOIN category AS cat  ON p.id_category = cat.Id_category_list  JOIN wish_list AS w ON w.Id_product = p.Id_product  WHERE w.Id_user=?`, [id])
  return rows;
}
