...
class ProductRepository extends AbstractRepository {
  ...
  async getProductByAsc(id) {
    const [rows] = await this.database.query(
      `SELECT p.* FROM ${this.table} AS p JOIN category AS cat on p.Id_category = cat.Id_category_list  where p.Id_category = ?  order by p.price asc`,
      [id]
    );
    return rows;
  }
  ...
  async getProductByDesc(id) {
    const [rows] = await this.database.query(
      `SELECT p.* FROM ${this.table} AS p JOIN category AS cat on p.Id_category = cat.Id_category_list  where p.Id_category = ?  order by p.price desc`,
      [id]
    );
    return rows;
  }
